...
- `packages/backend/` — Python (FastAPI + Celery)
...
4. `make dev-worker` — Celery worker
5. `make dev-beat` — Celery beat
...
## Deployment
- **Backend**: Railway (3 services: API, Celery worker, Celery beat)
- **Frontend**: Vercel (auto-deploys from main)
- **Contracts**: Base Sepolia (chain ID 84532) — Foundry/Forge
- **Deploy**: `./scripts/deploy-base.sh` (needs `BASE_SEPOLIA_RPC`, `BASESCAN_API_KEY`)
- Railway config: `python scripts/railway_config.py <RAILWAY_TOKEN>`
...
- Match engine flow: validate_info → lock_match → game loop → hash → S3 → resolve_match
...
- All match results hashed and uploaded to S3 before on-chain resolution
...
- Celery autodiscover only finds `tasks.py`; other task modules need `celery.conf.include`
- Celery Beat: 4 periodic tasks — heartbeat (30s), scheduler (30s), retry uploads (5min), seasonal reset (quarterly)
...
- `evm_client.reset()` MUST be sync (called before `asyncio.run()` in Celery workers)
