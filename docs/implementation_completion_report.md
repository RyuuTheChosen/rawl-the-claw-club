...
> **Historical document** from 2026-02-15. Post-report changes include: SF2 Genesis pivot (2026-02-16), inference pipeline update to 84x84 CnnPolicy (2026-02-17), and Railway/Vercel deployment (2026-02-18). See `launch_roadmap.md` for current status.
...
- Added stable-retro config: `retro_game`, `retro_integration_path`, `retro_obs_size`
...
**`packages/backend/src/rawl/engine/emulation/base.py`** (NEW)
- `EmulationEngine` ABC with `start()`, `step()`, `stop()` interface

**`packages/backend/src/rawl/engine/emulation/retro_engine.py`** (NEW)
- Wraps stable-retro (genesis_plus_gx core, SF2 Champion Edition Genesis) with format translation
- `_translate_obs()` — resize 200x256 → 256x256, wrap as `{P1, P2}` dict
- `_translate_info()` — re-nest flat keys into adapter-compatible nested dict (handles both SF2 `health`/`enemy_health` and prefixed `p1_health`/`p2_health` conventions)
- `_translate_action()` — concatenate `{P1, P2}` dict into flat MultiBinary(24) array
- Uses `Actions.FILTERED` (not MULTI_DISCRETE, which has a 2P bug on Genesis)
- Lazy `import retro` inside `start()` for testability

**`packages/backend/src/rawl/engine/diambra_manager.py`** (DELETED)
- Replaced by RetroEngine; DIAMBRA removed due to runtime auth dependency
...
- Full 8-step game loop:
  ...
  2. Start emulation engine via `RetroEngine` (stable-retro / Genesis)
...
**`packages/backend/src/rawl/training/worker.py`** (gutted)
- Training moved off-platform — users rent GPUs and run the `rawl-trainer` package
- Now raises `NotImplementedError` pointing to external training package
- Platform only validates submitted checkpoints
...
- 8 health checks (was 3, added 5):
  ...
  6. `check_retro()` — stable-retro ROM import check
...
- Game selector tabs (sf2ce is active launch title; sfiii3n, kof98, tektagt are future)
...
- Changed `match_format` type from `string` to `number` (backend returns int, not string)
...
1. **Game adapter stubs** — `umk3` and `doapp` still raise `NotImplementedError`; `sfiii3n` and `kof98`/`tektagt` remain in registry but are not active launch titles
