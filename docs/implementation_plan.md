...
- **Modify** `packages/backend/src/rawl/config.py` — add training tier limits, DIAMBRA docker settings, `solana_confirm_timeout`, `solana_max_retries`
...
**Goal:** Uncomment and implement the match runner game loop with DIAMBRA, model loading, frame publishing.

### Task 3.1 — Implement DIAMBRA manager
- **Modify** `packages/backend/src/rawl/engine/diambra_manager.py`
- `start()` — `diambra.arena.make()`, `env.reset()`, return `info`
- `step(action)` — passthrough to `env.step()`
- `stop()` — `env.close()`
...
- Load models → start DIAMBRA → validate info → lock match (oracle) → game loop:
...
### Task 4.2 — Training worker with progress streaming
- **Modify** `packages/backend/src/rawl/training/worker.py`
- Create DIAMBRA env, create PPO model, custom callback for progress
- Every 10K steps: publish to Redis stream `training:{job_id}:progress`
- Save model to S3, update `Fighter.model_path`

### Task 4.3 — Self-play training
- **Create** `packages/backend/src/rawl/training/self_play.py`
- `SelfPlayCallback`: maintain checkpoint pool, 70% self-play / 30% historical

### Task 4.4 — Training tier enforcement
...
- **Modify** `packages/backend/src/rawl/monitoring/health_checks.py` — add: Solana RPC, DIAMBRA gRPC, Celery workers, oracle keypair, model storage (5 missing of 8)
...
| `config.py` | Training tier, DIAMBRA, Solana config |
...
| `engine/diambra_manager.py` | Real DIAMBRA env management |
| `engine/match_runner.py` | Uncomment game loop |
| `training/validation.py` | Implement 4 validation steps |
| `training/worker.py` | Real PPO + progress streaming |
...
- **Phase 3**: Full match execution for sfiii3n with DIAMBRA locally
